---
layout: splash
title: "Flower Stand"
permalink: /flower-stand/
classes: full-width
---
<section class="lr-page-hero lr-page-hero--full reveal" style="--hero-image: url('{{ '/assets/images/flowerstand/IMG_1147.jpg' | relative_url }}');">
</section>

<section class="lr-section reveal">
  <div class="lr-stand-details">
    <h2>Seasonal Flower Stand</h2>
    <p>Open weekends for bouquet sales during the growing season (late June through October). Cut flower orders are taken and fulfilled seven days per week.</p>
    <h3>Details</h3>
    <ul>
      <li>Pre-order bouquets available for pickup.</li>
      <li>Grab-and-go bouquets sized for car cup holders: $10.</li>
      <li>Cash or Venmo accepted.</li>
      <li>4 Old Field Lane, Redding, CT.</li>
      <li>Weekend sales during the growing season (late June through October).</li>
    </ul>
  </div>
</section>

<section class="lr-section reveal">
  <div class="lr-gallery">
    <figure class="lr-gallery__item">
      <img src="{{ '/assets/images/flowerstand/IMG_0992.jpg' | relative_url }}" alt="Seasonal bouquet wrapped for pickup.">
    </figure>
    <figure class="lr-gallery__item">
      <img src="{{ '/assets/images/flowerstand/IMG_1091.jpg' | relative_url }}" alt="Buckets of fresh-cut flowers at the stand.">
    </figure>
    <figure class="lr-gallery__item">
      <img src="{{ '/assets/images/flowerstand/IMG_0509.jpg' | relative_url }}" alt="Fresh blooms available for weekend sales.">
    </figure>
  </div>
</section>

<section class="lr-section reveal">
  <h2>Find the Flower Stand</h2>
  <div class="lr-map">
    <iframe title="Map to The Little River Flower Farm" src="https://www.google.com/maps?q=4+Old+Field+Lane,+Redding,+CT&output=embed" loading="lazy" referrerpolicy="no-referrer-when-downgrade"></iframe>
  </div>
</section>
